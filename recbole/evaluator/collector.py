...
class Collector(object):
    ...
    def __init__(self, config):
        ...
        self.save_batch_users = []
        self.save_batch_item_length = []
    ...
    def eval_batch_collect(
        ...
        """ Collect the evaluation resource from batched eval data and batched model output.
            Args:
                ...
                positive_u(Torch.Tensor): the row index of positive items for each user. start from 0 to bs.
                positive_i(Torch.Tensor): the positive item id for each user. coz LOOV so only one pos item for each user
        """
        ...
        if self.register.need('rec.topk'):
            ...
            self.save_batch_users.append(interaction.interaction['user_id'])
            self.save_batch_item_length.append(interaction.interaction['item_length'])
