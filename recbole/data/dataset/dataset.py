...
class Dataset:
    ...
    def _load_data(self, token, dataset_path):
        ...
        self.item_feat = self._load_user_or_item_feat(token, dataset_path, FeatureSource.ITEM, 'iid_field') # returns attributes from.item file
    ...
    def _load_inter_feat(self, token, dataset_path):
        ...
        if self.benchmark_filename_list is None:
            ...
            # inter_feat = inter_feat[inter_feat['user_id']=='19']
    ...
    def _load_feat(self, filepath, source):
        ...
        seq_separator = self.config['seq_separator'] # below is to split the sequence, eg tags of items
    ...
    def _filter_by_inter_num(self):
        ...
        user_inter_num_interval = self._parse_intervals_str(self.config['user_inter_num_interval']) # get the num interaction interval from config
        ...
        user_inter_num = Counter(self.inter_feat[self.uid_field].values) if user_inter_num_interval else Counter() # count number of interactions for each user {'uid': count no}
        item_inter_num = Counter(self.inter_feat[self.iid_field].values) if item_inter_num_interval else Counter()

        while True: # need to repeatedly drop users and items
            ban_users = self._get_illegal_ids_by_inter_num( # get user ids whose interactions do not statisfy config
            ...
            # need to drop image and text as well like self.item_feat?
            # if self.config['additional_feat_suffix'] is not None:
            #     if 'image_feat' in self.feat_name_list:
            #         dropped_item = self.image_feat['iid_img'].isin(ban_items)
            #         self.image_feat.drop(self.image_feat.index[dropped_item], inplace=True)
            #     if 'text_feat' in self.feat_name_list:
            #         dropped_item = self.text_feat['iid_txt'].isin(ban_items)
            #         self.text_feat.drop(self.text_feat.index[dropped_item], inplace=True)

            dropped_inter = pd.Series(False, index=self.inter_feat.index)
            user_inter = self.inter_feat[self.uid_field] #(row index, 'uid') for all interactions
            item_inter = self.inter_feat[self.iid_field] #(row index, 'iid') for all interactions
            dropped_inter |= user_inter.isin(ban_users)
            dropped_inter |= item_inter.isin(ban_items)

            user_inter_num -= Counter(user_inter[dropped_inter].values) # count number of interactions for each user {'uid': count no} after removing dropped_inter
        ...
        # if self.config['additional_feat_suffix'] is not None:
        #     left_items = list(item_inter_num.keys())
        #     if 'tv' in self.config['additional_feat_suffix']:
        #         self.tv_feat = self.tv_feat[self.tv_feat['iid_txt'].isin(left_items)]
        #         assert self.tv_feat.shape[0] == len(item_inter_num)
        #     if 'image' in self.config['additional_feat_suffix']:
        #         self.image_feat = self.image_feat[self.image_feat['iid_img'].isin(left_items)]
        #         self.text_feat = self.text_feat[self.text_feat['iid_txt'].isin(left_items)]
        #         assert self.image_feat.shape[0] == len(item_inter_num)
        #     if 'desimgemb' in self.config['additional_feat_suffix']:
        #         # self.imgemb_feat = self.imgemb_feat[self.imgemb_feat['iid_img'].isin(left_items)]
        #         # self.desemb_feat = self.desemb_feat[self.desemb_feat['iid_des'].isin(left_items)]
        #         # self.titemb_feat = self.titemb_feat[self.titemb_feat['iid_tit'].isin(left_items)]
        #         # self.attremb_feat = self.attremb_feat[self.attremb_feat['iid_attr'].isin(left_items)]
        #         # self.desemb2_feat = self.desemb2_feat[self.desemb2_feat['iid_des2'].isin(left_items)]
        #         self.item_feat = self.item_feat[self.item_feat['item_id'].isin(left_items)]
        #         # assert self.imgemb_feat.shape[0] == len(item_inter_num) == self.desemb_feat.shape[0]
        #         self.desimgemb_feat = self.desimgemb_feat[self.desimgemb_feat['iid_desimg'].isin(left_items)]
        #         self.desimgemb2_feat = self.desimgemb2_feat[self.desimgemb2_feat['iid_desimg2'].isin(left_items)]
        #         assert self.desimgemb_feat.shape[0] == len(item_inter_num) == self.desimgemb2_feat.shape[0]
        #         # self.imgavgemb_feat = self.imgavgemb_feat[self.imgavgemb_feat['iid_imgavg'].isin(left_items)]
        #         # assert self.imgavgemb_feat.shape[0] == len(item_inter_num)
    ...
    def _remap(self, remap_list):
        ...
        if len(remap_list) == 0:
            return
        tokens, split_point = self._concat_remaped_tokens(remap_list) # concat item id from interaction, image and text files
        new_ids_list, mp = pd.factorize(tokens) # map each token to index
    ...
    def save(self, data_type):
        ...
        """
        save_dir = self.config['data_path']
        ensure_dir(save_dir)
        file = os.path.join(save_dir, f'{self.config["dataset"]}-{data_type}.pth')
