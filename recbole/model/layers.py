...
class ItemToInterestAggregation(nn.Module):
    ...
    def forward(self, input_tensor): # [B, L, d] -> [B, k, d]
        ...
        empty_mask = ~(input_tensor.abs().sum(dim=2).bool())
        D_matrix[empty_mask, :] = -np.inf
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x_q, x_kv, attention_mask):
        ...
        hidden_states = self.dense(context_layer)
        hidden_states = self.out_dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + x_kv)
