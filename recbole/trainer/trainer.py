...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.save_recomm_res = config['save_recomm_res']
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result, total_loss = self.evaluate(valid_data, load_best_model=False, show_progress=show_progress)
        valid_score = calculate_valid_score(valid_result, self.valid_metric)
        return valid_score, valid_result, total_loss
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=64, eta_min=0)
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score, valid_result, total_valid_loss = self._valid_epoch(valid_data, show_progress=show_progress)
                ...
                self.tensorboard.add_scalar('Vaild_loss', total_valid_loss, epoch_idx)
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            ...
            scores, loss = self.model.full_sort_predict(interaction.to(self.device))
        ...
        return interaction, scores, positive_u, positive_i, loss
    ...
    @torch.no_grad()
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False, save_name=None):
        r"""Evaluate the model based on the eval data.
        ...
        if load_best_model:
            ...
            self.model.load_state_dict(checkpoint['state_dict'])
            # self.model.load_state_dict({k: v for k, v in checkpoint['state_dict'].items() if
            #                    k not in ['desc_embedding.weight', 'img_embedding.weight']}, strict=False)
            self.model.load_other_parameter(checkpoint.get('other_parameter'))
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            interaction, scores, positive_u, positive_i, loss = eval_func(batched_data)
        ...
        if self.save_recomm_res:
            # recomm res
            rec_mat = struct.get('rec.topk')
            topk_idx, pos_len_list = torch.split(rec_mat, [max(self.topk), 1], dim=1)
            x_df = pd.DataFrame(topk_idx.cpu().numpy())
            # user id and item length for each user
            users = torch.cat(self.eval_collector.save_batch_users, dim=0).cpu().numpy() # user id sequence
            user_map = {v: k for k, v in iter_data.dataset.field2token_id['user_id'].items()}
            mapped_users = np.array([user_map[i] for i in list(users)])
            item_lengths = torch.cat(self.eval_collector.save_batch_item_length, dim=0).cpu().numpy() # each user buys how many items
            x_df.insert(0, 'uid', mapped_users)
            x_df.insert(1, 'item_length', item_lengths)
            x_df.columns = ['uid'] + ['item_length'] + ['top_' + str(i+1) for i in range(max(self.topk))]
            # x_df = x_df.astype(int)

            dir_name = os.path.abspath('recommend_topk/')
        ...
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            if (epoch_idx + 1) % self.save_step == 0:
                saved_model_file = os.path.join(
                    self.checkpoint_dir,
                    '{}-{}-v_{}-t_{}-{}-{}.pth'.format(self.config['model'], self.config['dataset'], self.config['has_v'], self.config['has_t'], str(epoch_idx + 1), get_local_time())
                )
